...
export function IronforgeDeliveryPage({ repository, api = deliveryApi, onRefresh }: Props) {
  ...
  return <GuidedWorkflow currentStep={step} description="选择 OUTPUT 交付包，提交管理员审核；合并后自动发布到铁炉堡。" nextDisabled={nextDisabled} nextLabel={step === 2 ? (busy ? '正在同步' : '同步图纸') : step === 5 ? (busy ? '正在提交' : '创建审核单') : '下一步'} onBack={step > 0 && !mrResult ? () => setStep((current) => current - 1) : undefined} onNext={mrResult ? undefined : step === 2 ? () => void sync() : step === 5 ? () => void createMr() : () => setStep((current) => current + 1)} steps={steps} title="提交图纸到铁炉堡">
    ...
    {mrResult ? <div className="wizard-success"><CheckCircle2 size={42} /><h2>已提交管理员审核</h2><p>GitLab MR !{mrResult.iid} 已创建。管理员合并后，图纸会发布到铁炉堡。</p><div className="success-actions">{mrResult.webUrl ? <a className="button button--secondary" href={mrResult.webUrl} rel="noreferrer" target="_blank">打开审核单</a> : null}<a className="button button--primary" href="http://ironforge.holo.tp/projects" rel="noreferrer" target="_blank">打开铁炉堡</a></div></div> : null}
    ...
    {!mrResult && step === 2 ? <div><Intro title="确认上传工程和图纸">软件会上传整个项目的有效改动，更新 charge.json，并包含选中的 OUTPUT 包。</Intro><dl className="confirm-list"><div><dt>本次操作项目</dt><dd>{repository.displayName}</dd></div><div><dt>本地文件夹</dt><dd>{repository.path}</dd></div><div><dt>当前分支</dt><dd>{repository.branch}</dd></div><div><dt>项目改动</dt><dd>{repository.changes.length} 个文件</dd></div><div><dt>交付包</dt><dd>{selectedPackages.size} 个</dd></div><div><dt>本次更新标题</dt><dd>{updateTitle}</dd></div><div><dt>本次交付标签</dt><dd>{tag || '不创建'}</dd></div></dl></div> : null}
